...
export async function fetchAndRegisterLocaleData(locale) {
  ...
  const timeoutPromise = new Promise((resolve, reject) =>
    window.setTimeout(() => reject(new Error('timed out fetching locale')), 5000),
  );
...
export function getLocalizedLanguageRegion(localeString, devtoolsLocale) {
  ...
  const languageInCurrentLocale = new Intl.DisplayNames([devtoolsLocale.locale], { type: 'language' }).of(
    localLanguage,
  );
  ...
  if (locale.region) {
    const regionInCurrentLocale = new Intl.DisplayNames([devtoolsLocale.locale], { type: 'region', style: 'short' }).of(
      locale.region,
    );
    const regionInTargetLocale = new Intl.DisplayNames([targetLanguage], { type: 'region', style: 'short' }).of(
      locale.region,
    );
