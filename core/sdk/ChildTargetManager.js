...
export class ChildTargetManager extends SDKModel {
  ...
  fireAvailableTargetsChanged() {
    TargetManager.instance().dispatchEventToListeners(TargetManagerEvents.AvailableTargetsChanged, [
  ...
  async createParallelConnection(onMessage) {
    ...
    const { connection, sessionId } = await this.createParallelConnectionAndSessionForTarget(
      this.#parentTarget,
      targetId,
    );
