...
export class DOMDebuggerModel extends SDKModel {
  ...
  async eventListeners(remoteObject) {
    ...
    for (const payload of listeners.listeners || []) {
      const location = this.#runtimeModelInternal
        .debuggerModel()
...
export class EventListener {
  ...
  isScrollBlockingType() {
    return (
      this.#typeInternal === 'touchstart' ||
      this.#typeInternal === 'touchmove' ||
      this.#typeInternal === 'mousewheel' ||
      this.#typeInternal === 'wheel'
    );
  }
}
...
export class DOMDebuggerManager {
  ...
  constructor() {
    ...
    this.#cspViolationsToBreakOn.push(
      new CSPViolationBreakpoint(
        i18nString(UIStrings.trustedTypeViolations),
        i18nString(UIStrings.sinkViolations),
    ...
    this.#cspViolationsToBreakOn.push(
      new CSPViolationBreakpoint(
        i18nString(UIStrings.trustedTypeViolations),
        i18nString(UIStrings.policyViolations),
    ...
    this.createEventListenerBreakpoints(
      i18nString(UIStrings.clipboard),
    ...
    this.createEventListenerBreakpoints(
      i18nString(UIStrings.dragDrop),
      ['drag', 'dragstart', 'dragend', 'dragenter', 'dragover', 'dragleave', 'drop'],
      ['*'],
    );
    this.createEventListenerBreakpoints(
      i18nString(UIStrings.keyboard),
  ...
  resolveEventListenerBreakpointInternal(eventName, eventTargetName) {
    ...
    for (const breakpoint of this.#eventListenerBreakpointsInternal) {
      ...
      if (
        eventName &&
        breakpoint.eventName === eventName &&
