...
export class NetworkDispatcher {
  ...
  finishNetworkRequest(networkRequest, finishTime, encodedDataLength, shouldReportCorbBlocking) {
    ...
    if (shouldReportCorbBlocking) {
      const message = i18nString(UIStrings.crossoriginReadBlockingCorb, {
        PH1: networkRequest.url(),
        PH2: networkRequest.mimeType,
      });
      this.#manager.dispatchEventToListeners(Events.MessageGenerated, {
        message: message,
    ...
    if (
      Common.Settings.Settings.instance().moduleSetting('monitoringXHREnabled').get() &&
...
export class MultitargetNetworkManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  isThrottling() {
    return (
      this.#networkConditionsInternal.download >= 0 ||
      this.#networkConditionsInternal.upload >= 0 ||
...
export class InterceptedRequest {
  ...
  async continueRequestWithContent(contentBlob, encoded, responseHeaders, isBodyOverridden) {
    ...
    MultitargetNetworkManager.instance().dispatchEventToListeners(
      MultitargetNetworkManager.Events.RequestFulfilled,
      this.request.url,
    );
