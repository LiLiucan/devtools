...
export class PageResourceLoader extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static instance(
    { forceNew, loadOverride, maxConcurrentLoads, loadTimeout } = {
      forceNew: false,
      loadOverride: null,
  ...
  async dispatchLoad(url, initiator) {
    ...
    const eligibleForLoadFromTarget =
      getLoadThroughTargetSetting().get() &&
      parsedURL &&
  ...
  getDeveloperResourceScheme(parsedURL) {
    ...
    switch (parsedURL.scheme) {
      ...
      case 'http':
        return isLocalhost
          ? Host.UserMetrics.DeveloperResourceScheme.SchemeHttpLocalhost
          : Host.UserMetrics.DeveloperResourceScheme.SchemeHttp;
      case 'https':
        return isLocalhost
          ? Host.UserMetrics.DeveloperResourceScheme.SchemeHttpsLocalhost
    ...
    try {
      ...
      return {
        ...
        errorDescription: {
          ...
          message:
            Host.ResourceLoader.netErrorToMessage(resource.netError, resource.httpStatusCode, resource.netErrorName) ||
            '',
          urlValid: undefined,
        },
      };
