...
export class RemoteObjectImpl extends RemoteObject {
  ...
  async setPropertyValue(name, value) {
    ...
    if (response.getError() || response.exceptionDetails) {
      return (
        response.getError() || (response.result.type !== 'string' ? response.result.description : response.result.value)
      );
    }
...
export class RemoteFunction {
  ...
  targetFunctionDetails() {
    ...
    function functionDetails(targetFunction) {
      const boundReleaseFunctionDetails = releaseTargetFunction.bind(
        null,
        this.#objectInternal !== targetFunction ? targetFunction : null,
      );
