...
export class Script {
  ...
  originalContentProvider() {
    return new TextUtils.StaticContentProvider.StaticContentProvider(this.contentURL(), this.contentType(), () =>
      this.requestContent(),
    );
  }
  async searchInContent(query, caseSensitive, isRegex) {
    ...
    return (matches.result || []).map(
      (match) => new TextUtils.ContentProvider.SearchMatch(match.lineNumber, match.lineContent),
    );
  }
  ...
  async editSource(newSource) {
    ...
    const response = await this.debuggerModel
      .target()
      .debuggerAgent()
      .invoke_setScriptSource({ scriptId: this.scriptId, scriptSource: newSource, allowTopFrameEditing: true });
