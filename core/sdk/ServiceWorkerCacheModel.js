...
export class ServiceWorkerCacheModel extends SDKModel {
  ...
  enable() {
    ...
    this.#securityOriginManager.addEventListener(
      SecurityOriginManagerEvents.SecurityOriginAdded,
      this.securityOriginAdded,
      this,
    );
    this.#securityOriginManager.addEventListener(
      SecurityOriginManagerEvents.SecurityOriginRemoved,
  ...
  async deleteCacheEntry(cache, request) {
    ...
    if (response.getError()) {
      Common.Console.Console.instance().error(
        i18nString(UIStrings.serviceworkercacheagentError, { PH1: cache.toString(), PH2: String(response.getError()) }),
      );
      return;
    }
  }
