...
ElementsTestRunner.expandAndDumpEventListeners = function (eventListenersView, callback, force) {
  ...
  if (force) {
    ...
    TestRunner.addSniffer(
      EventListeners.EventListenersView.EventListenersView.prototype,
...
ElementsTestRunner.expandAndDumpSelectedElementEventListeners = function (callback, force) {
  ElementsTestRunner.expandAndDumpEventListeners(
    ElementsTestRunner.eventListenersWidget().eventListenersView,
    callback,
    force,
  );
};
...
ElementsTestRunner.dumpStyleTreeItem = function (treeItem, prefix, depth, printInnerText) {
  const textContent = printInnerText
    ? treeItem.listItemElement.innerText
  ...
  if (
    treeItem.listItemElement.classList.contains('overloaded') ||
...
ElementsTestRunner.dumpRule = function (rule, currentIndent) {
  ...
  if (rule.type === 'import') {
    TestRunner.addResult(
      currentIndent +
        '@import: header=' +
        ElementsTestRunner.rangeText(rule.headerRange) +
        ', body=' +
        ElementsTestRunner.rangeText(rule.bodyRange),
    );
    ...
    if (!property.disabled) {
      TestRunner.addResult(
        currentIndent +
          ...
          property.value +
          "'" +
          (property.important ? ' is-important' : '') +
...
ElementsTestRunner.addNewRuleInStyleSheet = function (styleSheetHeader, selector, callback) {
  TestRunner.addSniffer(
    Elements.StylesSidebarPane.prototype,
    'addBlankSection',
...
ElementsTestRunner.addNewRule = function (selector, callback) {
  UI.panels.elements.stylesWidget.contentElement
    .querySelector('.styles-pane-toolbar')
