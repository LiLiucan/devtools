...
SourcesTestRunner.captureStackTraceIntoString = async function (callFrames, asyncStackTrace, options) {
  ...
  async function printCallFrames(callFrames, locationFunction, returnValueFunction) {
    ...
    for (let i = 0; i < callFrames.length; i++) {
      ...
      const isFramework = uiLocation
        ? self.Bindings.ignoreListManager.isUserIgnoreListedURL(uiLocation.uiSourceCode.url())
        : false;
  ...
  await printCallFrames(
    callFrames,
    SDK.DebuggerModel.CallFrame.prototype.location,
...
SourcesTestRunner.removeBreakpoint = function (sourceFrame, lineNumber) {
  ...
  const breakpointLocation = breakpointLocations.find(
    (breakpointLocation) =>
      breakpointLocation.uiLocation.uiSourceCode === sourceFrame.uiSourceCode() &&
...
SourcesTestRunner.createNewBreakpoint = async function (sourceFrame, lineNumber, condition, enabled) {
  ...
  const promise = new Promise((resolve) =>
    TestRunner.addSniffer(debuggerPlugin.__proto__, 'breakpointWasSetForTest', resolve),
  );
...
SourcesTestRunner.checkUILocation = function (uiSourceCode, lineNumber, columnNumber, location) {
  TestRunner.assertEquals(
    ...
    "Incorrect uiSourceCode, expected '" +
      (uiSourceCode ? uiSourceCode.url() : null) +
      "'," +
      " but got '" +
 ...
 */
SourcesTestRunner.runActionAndWaitForExactBreakpointDecorations = async function (
  sourceFrame,
  expectedDecorations,
  action,
  skipFirstWait = false,
) {
  const waitPromise = SourcesTestRunner.waitForExactBreakpointDecorations(
    sourceFrame,
    expectedDecorations,
    skipFirstWait,
  );
