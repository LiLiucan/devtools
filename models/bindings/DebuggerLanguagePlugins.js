...
async function resolveRemoteObject(
  ...
  const response = await callFrame.debuggerModel
    .target()
    .runtimeAgent()
...
async function formatSourceValue(callFrame, plugin, sourceType, base, field, evalOptions) {
  ...
  async function unpackResultObject(object) {
    const { tag, value, inspectableAddress, description } = await object.findProperties(
      'tag',
      'value',
      'inspectableAddress',
      'description',
  ...
  async doGetProperties(_ownProperties, accessorPropertiesOnly, _generatePreview) {
    ...
    if (typeInfo.members.length > 0) {
        ...
        for (let i = 0; i < typeInfo.arraySize; ++i) {
          ...
          properties.push(
            new SDK.RemoteObject.RemoteObjectProperty(
              name,
              await this.expandMember(elementTypeInfo, elementField),
              /* enumerable=*/ false,
              /* writable=*/ false,
              /* isOwn=*/ true,
              /* wasThrown=*/ false,
      ...
      const members = Promise.all(
        this.#sourceType.members.map(async (memberTypeInfo, idx) => {
    ...
    if (objectId) {
      ...
      const properties = extensionObjectProperties.map(
        (p) =>
          new SDK.RemoteObject.RemoteObjectProperty(
            p.name,
            new ExtensionRemoteObject(this.callFrame, p.value, this.plugin),
          ),
      );
...
export class DebuggerLanguagePluginManager {
  ...
  expandCallFrames(callFrames) {
    return Promise.all(
      callFrames.map(async (callFrame) => {
