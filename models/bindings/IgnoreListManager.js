  ...
  async updateScriptRanges(script, sourceMap) {
    ...
    if (!IgnoreListManager.instance().isUserIgnoreListedURL(script.sourceURL, script.isContentScript())) {
      hasIgnoreListedMappings =
        sourceMap
          ?.sourceURLs()
          .some((url) => this.isUserOrSourceMapIgnoreListedURL(url, sourceMap.hasIgnoreListHint(url))) ?? false;
    }
    ...
    const newRanges = sourceMap
      .findRanges((srcURL) => this.isUserOrSourceMapIgnoreListedURL(srcURL, sourceMap.hasIgnoreListHint(srcURL)), {
