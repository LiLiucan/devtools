...
export class FormatterWorkerPool {
  ...
  javaScriptIdentifiers(content) {
    return this.runTask('javaScriptIdentifiers' /* FormatterActions.FormatterActions.JAVASCRIPT_IDENTIFIERS */, {
      content: content,
    }).then((ids) => ids || []);
  }
  javaScriptSubstitute(expression, mapping) {
    return this.runTask('javaScriptSubstitute' /* FormatterActions.FormatterActions.JAVASCRIPT_SUBSTITUTE */, {
      content: expression,
      mapping: Array.from(mapping.entries()),
    }).then((result) => result || '');
  }
  javaScriptScopeTree(expression) {
    return this.runTask('javaScriptScopeTree' /* FormatterActions.FormatterActions.JAVASCRIPT_SCOPE_TREE */, {
      content: expression,
  ...
  outlineForMimetype(content, mimeType, callback) {
    switch (mimeType) {
      case 'text/html':
        this.runChunkedTask(
          'htmlOutline' /* FormatterActions.FormatterActions.HTML_OUTLINE */,
          { content: content },
          callback,
        );
        return true;
      case 'text/javascript':
        this.runChunkedTask(
          'javaScriptOutline' /* FormatterActions.FormatterActions.JAVASCRIPT_OUTLINE */,
          { content: content },
          callback,
        );
        return true;
