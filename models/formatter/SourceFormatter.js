...
export class SourceFormatter {
  ...
  constructor() {
    ...
    this.project = new Bindings.ContentProviderBasedProject.ContentProviderBasedProject(
      ...
      this.projectId,
      Workspace.Workspace.projectTypes.Formatter,
      'formatter',
  ...
  async format(uiSourceCode) {
    ...
    const resultPromise = new Promise(async (resolve, reject) => {
      ...
      try {
        const { formattedContent, formattedMapping } = await format(
          uiSourceCode.contentType(),
          uiSourceCode.mimeType(),
          content || '',
        );
        const cacheEntry = this.formattedSourceCodes.get(uiSourceCode);
...
class ScriptMapping {
  ...
  rawLocationToUILocation(rawLocation) {
    ...
    const [lineNumber, columnNumber] = formatData.mapping.originalToFormatted(
  ...
  uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber) {
    ...
    if (formatData.originalSourceCode.contentType().isScript()) {
      ...
      const rawLocations =
        Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().uiLocationToRawLocationsForUnformattedJavaScript(
    ...
    if (formatData.originalSourceCode.contentType() === Common.ResourceType.resourceTypes.Document) {
      ...
      if (debuggerModel) {
        const scripts = debuggerModel
          .scriptsForSourceURL(formatData.originalSourceCode.url())
        ...
        const locations = scripts
          .map((script) => script.rawLocation(originalLine, originalColumn))
          .filter((l) => Boolean(l));
  ...
  async setSourceMappingEnabled(formatData, enabled) {
    ...
    const updatePromises = scripts.map((script) =>
      Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().updateLocations(script),
    );
  ...
  scriptsForUISourceCode(uiSourceCode) {
    if (uiSourceCode.contentType() === Common.ResourceType.resourceTypes.Document) {
      ...
      if (debuggerModel) {
        const scripts = debuggerModel
          .scriptsForSourceURL(uiSourceCode.url())
          .filter((script) => script.isInlineScript() && !script.hasSourceURL);
        return scripts;
      }
    }
...
class StyleMapping {
  ...
  rawLocationToUILocation(rawLocation) {
    ...
    const formattedLocation = formatData.mapping.originalToFormatted(
      rawLocation.lineNumber,
      rawLocation.columnNumber || 0,
    );
  ...
  async setSourceMappingEnabled(formatData, enable) {
    ...
    const updatePromises = headers.map((header) =>
      Bindings.CSSWorkspaceBinding.CSSWorkspaceBinding.instance().updateLocations(header),
    );
    await Promise.all(updatePromises);
  }
  headersForUISourceCode(uiSourceCode) {
    if (uiSourceCode.contentType() === Common.ResourceType.resourceTypes.Document) {
      ...
      if (cssModel) {
        return cssModel
          .headersForSourceURL(uiSourceCode.url())
