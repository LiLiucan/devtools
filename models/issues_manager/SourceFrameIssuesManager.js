...
export class SourceFrameIssuesManager {
  ...
  constructor(issuesManager) {
    ...
    this.issuesManager.addEventListener(
      'FullUpdateRequired' /* Events.FullUpdateRequired */,
  ...
  #addIssue(issue) {
    ...
    if (srcLocation && debuggerModel) {
      const rawLocation = debuggerModel.createRawLocationByURL(
        srcLocation.url,
        srcLocation.lineNumber,
        srcLocation.columnNumber,
      );
      if (rawLocation) {
  ...
  #isTrustedTypeIssue(issue) {
    return (
      (issue instanceof ContentSecurityPolicyIssue && issue.code() === trustedTypesSinkViolationCode) ||
