...
export class NetworkPersistenceManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  canSaveUISourceCodeForOverrides(uiSourceCode) {
    return (
      ...
      !this.bindings.has(uiSourceCode) &&
      !this.savingForOverrides.has(uiSourceCode)
    );
  }
  ...
  async interceptionHandler(interceptedRequest) {
    ...
    if (fileSystemUISourceCode) {
      ...
      if (!responseBody.error && responseBody.content) {
        void interceptedRequest.continueRequestWithContent(
          new Blob([responseBody.content], { type: mimeType }),
          /* encoded */ true,
          responseHeaders,
          /* isBodyOverridden */ false,
        );
...
export function isHeaderOverride(arg) {
  ...
  return arg.headers.every(
    (header) => header.name && typeof header.name === 'string' && typeof header.value === 'string',
  );
}
