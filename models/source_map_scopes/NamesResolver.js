...
const computeScopeTree = async function (functionScope) {
  ...
  const scopeRange = new TextUtils.TextRange.TextRange(
    functionStartLocation.lineNumber,
    functionStartLocation.columnNumber,
    functionEndLocation.lineNumber,
  ...
  let suffix = '}';
  let scopeTree = await Formatter.FormatterWorkerPool.formatterWorkerPool().javaScriptScopeTree(
    prefix + scopeText + suffix,
  );
  if (!scopeTree) {
    // Try to parse the function as an arrow function.
    prefix = '';
    suffix = '';
    scopeTree = await Formatter.FormatterWorkerPool.formatterWorkerPool().javaScriptScopeTree(
      prefix + scopeText + suffix,
    );
  }
  if (!scopeTree) {
...
const resolveScope = async (scope) => {
  ...
  function findFunctionScope() {
    ...
    for (scopeIndex; scopeIndex < scopeChain.length; scopeIndex++) {
      ...
      if (
        kind === 'local' /* Protocol.Debugger.ScopeType.Local */ ||
        kind === 'closure' /* Protocol.Debugger.ScopeType.Closure */
      ) {
        break;
      }
    }
...
export const resolveScopeInObject = function (scope) {
  ...
  if (
    ...
    startLocationScript !== endLocation.script()
  ) {
    return scope.object();
  }
