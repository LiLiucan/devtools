...
export class AXNodePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  appendRelatedNode(relatedNode, _index) {
    ...
    const nodeTreeElement = new AXRelatedNodeSourceTreeElement(
      { deferredNode: deferredNode, idref: undefined },
      relatedNode,
    );
...
export class AXValueSourceTreeElement extends AXNodePropertyTreeElement {
  ...
  appendRelatedNodeWithIdref(relatedNode, idref) {
    ...
    const nodeTreeElement = new AXRelatedNodeSourceTreeElement(
      { deferredNode: deferredNode, idref: idref },
      relatedNode,
    );
    this.appendChild(nodeTreeElement);
  }
  ...
  render() {
    ...
    if (this.deferredNode) {
      ...
      void this.deferredNode.resolvePromise().then((node) => {
        void Common.Linkifier.Linkifier.linkify(node, { tooltip: undefined, preventKeyboardFocus: true }).then(
          (linkfied) => valueElement.appendChild(linkfied),
        );
      });
    } else if (this.idref) {
  ...
  static createReasonElement(reason, axNode) {
    ...
    switch (reason) {
      ...
      case 'ariaHiddenElement': {
        ...
        reasonElement = i18n.i18n.getFormatLocalizedString(str_, UIStrings.elementIsPlaceholder, {
          PH1: ariaHiddenSpan,
        });
        break;
      }
      case 'ariaHiddenSubtree': {
        ...
        reasonElement = i18n.i18n.getFormatLocalizedString(str_, UIStrings.placeholderIsPlaceholderOnAncestor, {
          PH1: ariaHiddenSpan,
          PH2: trueSpan,
        });
        break;
      ...
      case 'presentationalRole': {
        ...
        const rolePresentationSpan = (document.createElement('span', { is: 'source-code' }).textContent =
          'role=' + role);
        reasonElement = i18n.i18n.getFormatLocalizedString(str_, UIStrings.elementHasPlaceholder, {
