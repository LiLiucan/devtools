...
export class AnimationTimeline extends UI.Widget.VBox {
  ...
  createHeader() {
    ...
    this.#pauseButton = new UI.Toolbar.ToolbarToggle(
      i18nString(UIStrings.pauseAll),
      'largeicon-pause',
      'largeicon-resume',
    );
    ...
    UI.UIUtils.installDragHandle(
      gridHeader,
      this.repositionScrubber.bind(this),
  ...
  addAnimationGroup(group) {
    ...
    UI.ARIAUtils.setAccessibleName(
      preview.element,
      i18nString(UIStrings.animationPreviewS, { PH1: this.#groupBuffer.indexOf(group) + 1 }),
    );
  ...
  scrubberDragMove(event) {
    ...
    const currentTime = Math.max(
      0,
      Math.min((this.#originalScrubberTime || 0) + delta / this.pixelMsRatio(), this.duration()),
    );
    if (this.#scrubberPlayer) {
      this.#scrubberPlayer.currentTime = currentTime;
    }
