...
export class AnimationUI {
  ...
  constructor(animation, timeline, parentElement) {
    ...
    UI.UIUtils.installDragHandle(
      ...
      this.mouseDown.bind(this, 'AnimationDrag' /* Events.AnimationDrag */, null),
    ...
    AnimationUI.installDragHandleKeyboard(
      this.#activeIntervalGroup,
      this.keydownMove.bind(this, 'AnimationDrag' /* Events.AnimationDrag */, null),
    );
    this.#cachedElements = [];
    this.#movementInMs = 0;
  ...
  drawAnimationLine(iteration, parentElement) {
    ...
    cache.animationLine.setAttribute(
      'x2',
      (this.duration() * this.#timeline.pixelMsRatio() + Options.AnimationMargin).toFixed(2),
    );
  }
  ...
  redraw() {
    ...
    for (
      iteration = 1;
      iteration < this.#animationInternal.source().iterations() &&
  ...
  renderTransition() {
    ...
    this.renderKeyframe(
      ...
      this.duration() * this.#timeline.pixelMsRatio(),
    ...
    this.drawPoint(
      0,
      activeIntervalGroup,
      this.duration() * this.#timeline.pixelMsRatio() + Options.AnimationMargin,
      -1,
      true,
    );
  }
  ...
  delay() {
    ...
    if (
      this.#mouseEventType === 'AnimationDrag' /* Events.AnimationDrag */ ||
