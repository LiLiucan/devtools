...
export class AppManifestView extends UI.Widget.VBox {
  ...
  async updateManifest(immediately) {
    ...
    void this.throttler.schedule(
      () => this.renderManifest(url, data, errors, installabilityErrors, manifestIcons, appId),
      immediately,
    );
  }
  async renderManifest(url, data, errors, installabilityErrors, manifestIcons, appIdResponse) {
    ...
    if (appId && recommendedId) {
      ...
      if (!stringProperty('id')) {
        ...
        suggestedIdNote.appendChild(
          i18n.i18n.getFormatLocalizedString(str_, UIStrings.appIdNote, {
            PH1: noteSpan,
            PH2: idSpan,
            PH3: startUrlSpan,
            PH4: idSpan2,
    ...
    this.iconsSection
      .appendRow()
      .appendChild(i18n.i18n.getFormatLocalizedString(str_, UIStrings.needHelpReadOurS, { PH1: documentationLink }));
    ...
    for (const screenshot of screenshots) {
      ...
      const { imageResourceErrors: screenshotErrors } = await this.appendImageResourceToSection(
        url,
        screenshot,
        screenshotSection,
        /** isScreenshot= */ true,
