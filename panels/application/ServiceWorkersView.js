...
export class ServiceWorkersView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const seeOthers = UI.Fragment
      .html`<a class="devtools-link" role="link" tabindex="0" href="chrome://serviceworker-internals" target="_blank" style="display: inline; cursor: pointer;">${i18nString(
      UIStrings.seeAllRegistrations,
    )}</a>`;
    ...
    const drawerChangeHandler = (event) => {
      ...
      if (this.manager && !isDrawerOpen) {
        const {
          serviceWorkerNetworkRequestsPanelStatus: { isOpen, openedAt },
        } = this.manager;
        if (isOpen) {
...
export class Section {
  ...
  constructor(manager, section, registration) {
    ...
    this.updateButton = new UI.Toolbar.ToolbarButton(
  ...
  update() {
    ...
    if (installing) {
      ...
      if (!this.targetForVersionId(installing.id) && (installing.isRunning() || installing.isStarting())) {
        this.createLink(
          installingEntry,
          i18nString(UIStrings.inspect),
          this.inspectButtonClicked.bind(this, installing.id),
        );
      }
    }
  ...
  updateClientInfo(element, targetInfo) {
    ...
    this.createLink(
      ...
      this.activateTarget.bind(this, targetInfo.targetId),
