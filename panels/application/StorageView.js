...
export class StorageView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  targetAdded(target) {
    ...
    storageKeyManager.addEventListener(
      SDK.StorageKeyManager.Events.MainStorageKeyChanged,
  ...
  static clear(target, securityOrigin, selectedStorageTypes, includeThirdPartyCookies) {
    void target
      .storageAgent()
      .invoke_clearDataForOrigin({ origin: securityOrigin, storageTypes: selectedStorageTypes.join(',') });
  ...
  async doUpdate() {
    ...
    const quota = quotaOverridden
      ? UI.Fragment.Fragment.build`<b>${formattedQuotaAsString}</b>`.element()
      : quotaAsString;
    const element = i18n.i18n.getFormatLocalizedString(str_, UIStrings.storageQuotaUsed, {
      PH1: usageAsString,
      PH2: quota,
    });
