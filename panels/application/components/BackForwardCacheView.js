...
export class BackForwardCacheView extends HTMLElement {
  ...
  #renderMainFrameInformation() {
    ...
    return LitHtml.html`
      ...
      ${this.#maybeRenderExplanations(
        this.#frame.backForwardCacheDetails.explanations,
  ...
  #maybeRenderFrameTree(explanationTree) {
    if (
      !explanationTree ||
      (explanationTree.explanations.length === 0 && explanationTree.children.length === 0) ||
  ...
  #maybeRenderExplanations(explanations, explanationTree) {
    ...
    const supportPending = explanations.filter(
      (explanation) =>
        explanation.type === 'SupportPending' /* Protocol.Page.BackForwardCacheNotRestoredReasonType.SupportPending */,
    );
    const circumstantial = explanations.filter(
      (explanation) =>
        explanation.type === 'Circumstantial' /* Protocol.Page.BackForwardCacheNotRestoredReasonType.Circumstantial */,
    );
  ...
  #maybeRenderReasonContext(explanation) {
    if (
      ...
      <${ChromeLink.ChromeLink.ChromeLink.litTagName} .href=${link}>${explanation.context}</${
