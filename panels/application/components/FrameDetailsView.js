...
export class FrameDetailsView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  constructor(frame) {
    ...
    frame
      .resourceTreeModel()
      .addEventListener(SDK.ResourceTreeModel.Events.PrerenderingStatusUpdated, this.update, this);
  }
  async doUpdate() {
...
export class FrameDetailsReportView extends HTMLElement {
      ...
      LitHtml.render(
        ...
        <${ReportView.ReportView.Report.litTagName} .data=${{ reportTitle: this.#frame.displayName() }}>
          ...
          ${LitHtml.Directives.until(
            this.#permissionsPolicies?.then((policies) => {
  ...
  #maybeRenderSourcesLinkForURL() {
    ...
    return renderIconLink('sources_panel_icon', i18nString(UIStrings.clickToRevealInSourcesPanel), () =>
  ...
  async #renderOwnerElement() {
    if (this.#frame) {
      ...
      if (linkTargetDOMNode) {
        ...
        // clang-format off
        return LitHtml.html`
            <${ReportView.ReportView.ReportKey.litTagName}>${i18nString(UIStrings.ownerElement)}</${
          ReportView.ReportView.ReportKey.litTagName
        }>
  ...
  #maybeRenderCreationStacktrace() {
    ...
    if (creationStackTraceData && creationStackTraceData.creationStackTrace) {
      ...
      // clang-format off
      return LitHtml.html`
        <${ReportView.ReportView.ReportKey.litTagName} title=${i18nString(
        UIStrings.creationStackTraceExplanation,
  ...
  #renderIsolationSection() {
    ...
    return LitHtml.html`
      ...
      <${ReportView.ReportView.ReportValue.litTagName}>
        ...
        }\xA0${this.#maybeRenderSecureContextExplanation()}
      </${ReportView.ReportView.ReportValue.litTagName}>
  ...
  #maybeRenderCrossOriginStatus(info, policyName, noneValue) {
    ...
    return LitHtml.html`
      ...
      <${ReportView.ReportView.ReportValue.litTagName}>
        ...
        ${
          endpoint
            ? LitHtml.html`<span class="inline-name">${i18nString(UIStrings.reportingTo)}</span>${endpoint}`
            : LitHtml.nothing
        }
      </${ReportView.ReportView.ReportValue.litTagName}>
    `;
        ...
        function renderHint(frame) {
          switch (frame.getCrossOriginIsolatedContextType()) {
            ...
            case 'NotIsolated' /* Protocol.Page.CrossOriginIsolatedContextType.NotIsolated */:
              if (sabAvailable) {
                return LitHtml.html`<span class="inline-comment">${i18nString(
                  UIStrings.willRequireCrossoriginIsolated,
                )}</span>`;
              }
              return LitHtml.html`<span class="inline-comment">${i18nString(
                UIStrings.requiresCrossoriginIsolated,
              )}</span>`;
            case 'NotIsolatedFeatureDisabled' /* Protocol.Page.CrossOriginIsolatedContextType.NotIsolatedFeatureDisabled */:
              if (!sabTransferAvailable) {
                return LitHtml.html`<span class="inline-comment">${i18nString(
                  UIStrings.transferRequiresCrossoriginIsolatedPermission,
  ...
  #renderMeasureMemoryAvailability() {
    if (this.#frame) {
      ...
      return LitHtml.html`
        ...
        <${ReportView.ReportView.ReportValue.litTagName}>
          <span title=${tooltipText}>${availabilityText}</span>\xA0<x-link class="link" href="https://web.dev/monitor-total-page-memory-usage/">${i18nString(
        UIStrings.learnMore,
      )}</x-link>
        </${ReportView.ReportView.ReportValue.litTagName}>
      `;
