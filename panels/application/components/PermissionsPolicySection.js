...
export class PermissionsPolicySection extends HTMLElement {
  ...
  #renderAllowed() {
    const allowed = this.#permissionsPolicySectionData.policies
      .filter((p) => p.allowed)
      .map((p) => p.feature)
      .sort();
    if (!allowed.length) {
  ...
  async #renderDisallowed() {
    const disallowed = this.#permissionsPolicySectionData.policies
      .filter((p) => !p.allowed)
      ...
      disallowed.map(async (policy) => {
        ...
        <div class="permissions-row">
          ...
          <div>
            ${
              linkTargetDOMNode
                ? renderIconLink('elements_panel_icon', i18nString(UIStrings.clickToShowIframe), () =>
            ...
            ${
              linkTargetRequest
                ? renderIconLink('network_panel_icon', i18nString(UIStrings.clickToShowHeader), revealHeader)
                : LitHtml.nothing
            }
          </div>
        </div>
