...
export class ProtocolHandlersView extends HTMLElement {
  ...
  #renderProtocolTest() {
    ...
    const protocolOptions = this.#protocolHandlers
      .filter((p) => p.protocol)
      .map((p) => LitHtml.html`<option value=${p.protocol}>${p.protocol}://</option>`);
    return LitHtml.html`
       <div class="protocol-handlers-row">
        <select class="chrome-select protocol-select" @change=${this.#handleProtocolSelect} aria-label=${i18nString(
      UIStrings.dropdownLabel,
    )}>
           ${protocolOptions}
        </select>
      ...
      this.#handleQueryInputChange
        ...
        <${Buttons.Button.Button.litTagName} .variant=${'primary' /* Buttons.Button.Variant.PRIMARY */} @click=${
  ...
  connectedCallback() {
    this.#shadow.adoptedStyleSheets = [protocolHandlersViewStyles, inspectorCommonStyles, Input.textInputStyles];
  }
  #render() {
