...
export class ObjectEventListenersSidebarPane extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.#refreshButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.refreshGlobalListeners),
      'largeicon-refresh',
    );
  ...
  update() {
    ...
    void Promise.all([this.windowObjectInContext(executionContext)]).then(
      this.#eventListenersView.addObjects.bind(this.#eventListenersView),
    );
  }
  wasShown() {
    super.wasShown();
