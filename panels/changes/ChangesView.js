...
function diffStats(diff) {
  const insertions = diff.reduce(
    (ins, token) => ins + (token[0] === Diff.Diff.Operation.Insert ? token[1].length : 0),
    0,
  );
  const deletions = diff.reduce(
    (ins, token) => ins + (token[0] === Diff.Diff.Operation.Delete ? token[1].length : 0),
    0,
  );
...
export class ChangesView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.changesSidebar.addEventListener(
      'SelectedUISourceCodeChanged' /* Events.SelectedUISourceCodeChanged */,
    ...
    const revertButton = new UI.Toolbar.ToolbarButton(
      i18nString(UIStrings.revertAllChangesToCurrentFile),
      'largeicon-undo',
    );
  ...
  click(event) {
    ...
    for (const target of event.composedPath()) {
      ...
      if (target.classList.contains('diff-line-content') && target.hasAttribute('data-line-number')) {
        ...
        if (
          Root.Runtime.experiments.isEnabled(Root.Runtime.ExperimentName.PRECISE_CHANGES) &&
