...
export class CoverageView extends UI.Widget.VBox {
  ...
  buildLandingPage() {
    ...
    if (this.startWithReloadButton) {
      this.inlineReloadButton = UI.UIUtils.createInlineButton(
        UI.Toolbar.Toolbar.createActionButtonForId('coverage.start-with-reload'),
      );
      message = i18n.i18n.getFormatLocalizedString(str_, UIStrings.clickTheReloadButtonSToReloadAnd, {
  ...
  async startRecording(options) {
    ...
    if (
      (this.startWithReloadButton && this.startWithReloadButton.element.hasFocus()) ||
