...
export class CSSOverviewCompletedView extends UI.Panel.PanelWithSidebar {
  ...
  constructor(controller) {
    ...
    this.#controller.addEventListener(
      'PopulateNodes' /* CSSOverViewControllerEvents.PopulateNodes */,
      this.#createElementsView,
      this,
    );
  ...
  #onClick(evt) {
    ...
    evt.consume();
    this.#controller.dispatchEventToListeners('PopulateNodes' /* CSSOverViewControllerEvents.PopulateNodes */, {
      payload,
    });
  ...
  async #render(data) {
    ...
    <div class="vbox overview-completed-view">
      ...
      <div $="font-info" class="results-section font-info">
        ...
        ${
          fontInfo.size > 0
            ? this.#fontInfoToFragment(fontInfo)
      ...
      <div $="unused-declarations" class="results-section unused-declarations">
        ...
        ${
          unusedDeclarations.size > 0
            ? this.#groupToFragment(unusedDeclarations, 'unused-declarations', 'declaration')
            : UI.Fragment.Fragment.build`<div class="horizontally-padded">${i18nString(
                UIStrings.thereAreNoUnusedDeclarations,
              )}</div>`
        }
      </div>
      ...
      <div $="media-queries" class="results-section media-queries">
        ...
        ${
          mediaQueries.size > 0
            ? this.#groupToFragment(mediaQueries, 'media-queries', 'text')
            : UI.Fragment.Fragment.build`<div class="horizontally-padded">${i18nString(
                UIStrings.thereAreNoMediaQueries,
              )}</div>`
        }
      </div>
    </div>`;
  ...
  ${fonts.map(([font, fontMetrics]) => {
    return UI.Fragment.Fragment.build`<section class="font-family"><h2>${font}</h2> ${this.#fontMetricsToFragment(
      font,
      fontMetrics,
    )}</section>`;
  })}
  ...
  #contrastIssueToFragment(key, issues) {
    ...
    const blockFragment = UI.Fragment.Fragment.build`<li>
      ...
      <div class="block-title">
        <div class="contrast-warning hidden" $="aa"><span class="threshold-label">${i18nString(
          UIStrings.aa,
        )}</span></div>
        <div class="contrast-warning hidden" $="aaa"><span class="threshold-label">${i18nString(
          UIStrings.aaa,
        )}</span></div>
        <div class="contrast-warning hidden" $="apca"><span class="threshold-label">${i18nString(
          UIStrings.apca,
        )}</span></div>
      </div>
    </li>`;
...
export class ElementDetailsView extends UI.Widget.Widget {
  ...
  constructor(controller, domModel, cssModel, linkifier) {
    ...
    this.#elementGrid.addEventListener(
      DataGrid.DataGrid.Events.SortingChanged,
  ...
  #onMouseOver(evt) {
    ...
    this.#controller.dispatchEventToListeners(
      'RequestNodeHighlight' /* CSSOverViewControllerEvents.RequestNodeHighlight */,
      backendNodeId,
    );
  }
