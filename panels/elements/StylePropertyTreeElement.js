...
export class StylePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  innerUpdateTitle() {
    ...
    if (this.property.name.startsWith('--') && this.nameElement) {
      UI.Tooltip.Tooltip.install(
        this.nameElement,
        this.matchedStylesInternal.computeCSSVariable(this.style, this.property.name) || '',
      );
    }
    ...
    if (this.property.parsedOk) {
      ...
      this.listItemElement.insertBefore(
        StylesSidebarPane.createExclamationMark(this.property, null),
  ...
  async updateFontVariationSettingsWarning() {
    ...
    for (const elementSetting of variationSettings) {
      for (const font of matchingFontFaces) {
        ...
        if (elementSetting.value < fontSetting.minValue || elementSetting.value > fontSetting.maxValue) {
          warnings.push(
            i18nString(UIStrings.valueForSettingSSIsOutsideThe, {
    ...
    this.listItemElement.insertBefore(
      StylesSidebarPane.createExclamationMark(this.property, warnings.join(' ')),
      this.listItemElement.firstChild,
    );
  }
  mouseUp(event) {
  ...
  createCopyContextMenu(event) {
    ...
    contextMenu
      .headerSection()
      .appendItem(i18nString(UIStrings.copyCssDeclarationAsJs), this.copyCssDeclarationAsJs.bind(this));
    ...
    contextMenu
      .clipboardSection()
      .appendItem(i18nString(UIStrings.copyAllCssDeclarationsAsJs), this.copyAllCssDeclarationAsJs.bind(this));
