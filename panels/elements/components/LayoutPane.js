...
export class LayoutPane extends HTMLElement {
  ...
  constructor() {
    super();
    this.#shadow.adoptedStyleSheets = [Input.checkboxStyles, layoutPaneStyles, inspectorCommonStyles];
  }
  set data(data) {
  ...
  #render() {
    ...
    render(
      html`
        <details open>
          <summary class="header" @keydown=${this.#onSummaryKeyDown}>${i18nString(UIStrings.grid)}</summary>
        ...
        ${this.#flexContainerElements !== undefined
          ? html`
              <details open>
                <summary class="header" @keydown=${this.#onSummaryKeyDown}>${i18nString(UIStrings.flexbox)}</summary>
  ...
  #renderElement(element) {
    ...
    return html`<div class="element">
      ...
      <label @keyup=${onColorLabelKeyUp} @keydown=${onColorLabelKeyDown} tabindex="0" title=${i18nString(
      UIStrings.chooseElementOverlayColor,
    )} aria-label=${i18nString(UIStrings.chooseElementOverlayColor)} class="color-picker-label" style="background: ${
      element.color
    };">
        <input @change=${onColorChange} @input=${onColorChange} tabindex="-1" class="color-picker" type="color" value=${
      element.color
    } />
      </label>
      <button tabindex="0" @click=${onElementClick} title=${i18nString(
      UIStrings.showElementInTheElementsPanel,
    )} class="show-element"></button>
    </div>`;
    // clang-format on
  }
