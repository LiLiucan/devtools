...
export class DeviceModeToolbar {
  ...
  constructor(model, showMediaInspectorSetting, showRulersSetting) {
    ...
    this.showDeviceScaleFactorSetting = Common.Settings.Settings.instance().createSetting(
      'emulation.showDeviceScaleFactor',
      false,
    );
    ...
    this.emulatedDevicesList.addEventListener(
      'CustomDevicesUpdated' /* EmulationModel.EmulatedDevices.Events.CustomDevicesUpdated */,
      this.deviceListChanged,
      this,
    );
    this.emulatedDevicesList.addEventListener(
      'StandardDevicesUpdated' /* EmulationModel.EmulatedDevices.Events.StandardDevicesUpdated */,
      this.deviceListChanged,
      this,
    );
    this.persistenceSetting = Common.Settings.Settings.instance().createSetting('emulation.deviceModeValue', {
      device: '',
      orientation: '',
      mode: '',
    });
    ...
    function appendScaleItem(title, value) {
      contextMenu
        ...
        .appendCheckboxItem(
          ...
          this.model.scaleSetting().get() === value,
          false,
        );
    }
  }
  ...
  appendDeviceScaleMenuItems(contextMenu) {
    ...
    const defaultValue =
      this.model.uaSetting().get() === EmulationModel.DeviceModeModel.UA.Mobile ||
  ...
  appendUserAgentMenuItems(contextMenu) {
    ...
    function appendUAItem(title, value) {
      contextMenu
        .defaultSection()
        .appendCheckboxItem(title, uaSetting.set.bind(uaSetting, value), uaSetting.get() === value);
    }
  }
  ...
  wrapToolbarItem(element) {
    ...
    const shadowRoot = UI.Utils.createShadowRootWithCoreStyles(container, {
      cssFile: deviceModeToolbarStyles,
  ...
  appendDeviceMenuItems(contextMenu) {
    ...
    function appendGroup(devices) {
      ...
      for (const device of devices) {
        section.appendCheckboxItem(
          device.title,
          this.emulateDevice.bind(this, device),
  ...
  modeMenuClicked(event) {
    ...
    if (
      (device.isDualScreen || device.modes.length === 2) &&
