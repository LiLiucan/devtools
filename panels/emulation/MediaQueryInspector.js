...
export class MediaQueryInspector extends UI.Widget.Widget {
  ...
  constructor(getWidthCallback, setWidthCallback) {
    ...
    UI.ZoomManager.ZoomManager.instance().addEventListener(
      'ZoomChanged' /* UI.ZoomManager.Events.ZoomChanged */,
  ...
  onContextMenu(event) {
    ...
    for (let i = 0; i < locations.length; ++i) {
      const uiLocation = Bindings.CSSWorkspaceBinding.CSSWorkspaceBinding.instance().rawLocationToUILocation(
        locations[i],
      );
      if (!uiLocation) {
        continue;
      ...
      marker
        .createChild(
          ...
          'media-inspector-marker-label-container ' +
            (atLeft ? 'media-inspector-marker-label-container-left' : 'media-inspector-marker-label-container-right'),
        )
        .createChild(
          'span',
...
export class MediaQueryUIModel {
  ...
  dimensionsEqual(other) {
    ...
    const minWidthEqual =
      !thisMinWidthExpression || thisMinWidthExpression.computedLength() === otherMinWidthExpression?.computedLength();
    const maxWidthEqual =
      !thisMaxWidthExpression || thisMaxWidthExpression.computedLength() === otherMaxWidthExpression?.computedLength();
