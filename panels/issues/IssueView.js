...
const issueTypeToNetworkHeaderMap = new Map([
  [IssuesManager.Issue.IssueCategory.Cookie, NetworkForward.UIRequestLocation.UIRequestTabs.Cookies],
  [IssuesManager.Issue.IssueCategory.CrossOriginEmbedderPolicy, NetworkForward.UIRequestLocation.UIRequestTabs.Headers],
  [IssuesManager.Issue.IssueCategory.MixedContent, NetworkForward.UIRequestLocation.UIRequestTabs.Headers],
]);
...
export class IssueView extends UI.TreeOutline.TreeElement {
  ...
  #createReadMoreLinks() {
    ...
    for (const description of this.#description.links) {
      const link = UI.Fragment.html`<x-link class="link devtools-link" tabindex="0" href=${
        description.link
      }>${i18nString(UIStrings.learnMoreS, { PH1: description.linkTitle })}</x-link>`;
