...
export class RequestHeadersComponent extends HTMLElement {
  ...
  #render() {
    ...
    // clang-format off
    render(
      html` ${this.#renderGeneralSection()} ${this.#renderResponseHeaders()} ${this.#renderRequestHeaders()} `,
      this.#shadow,
      { host: this },
    );
    // clang-format on
  ...
  #renderHeaderOverridesLink() {
    ...
    // Disabled until https://crbug.com/1079231 is fixed.
    // clang-format off
    const fileIcon = overridesSetting.get()
      ? html`
  ...
  #getHeaderOverridesFileUrl() {
    ...
    return (
      fileUrl.substring(0, fileUrl.lastIndexOf('/')) + '/' + Persistence.NetworkPersistenceManager.HEADERS_FILENAME
    );
  }
  ...
  #renderRawHeaders(rawHeadersText, forResponseHeaders) {
    ...
    return html`
      <div
        ...
        on-render=${ComponentHelpers.Directives.nodeRenderedCallback(addContextMenuListener)}
      >
  ...
  #renderGeneralSection() {
    ...
    return html`
      ...
      >
        ...
        ${
          this.#request.statusCode
            ? this.#renderGeneralRow(i18nString(UIStrings.requestMethod), this.#request.requestMethod)
            : LitHtml.nothing
        }
        ${
          this.#request.statusCode
            ? this.#renderGeneralRow(i18nString(UIStrings.statusCode), statusText, statusClasses)
            : LitHtml.nothing
        }
        ${
          this.#request.remoteAddress()
            ? this.#renderGeneralRow(i18nString(UIStrings.remoteAddress), this.#request.remoteAddress())
            : LitHtml.nothing
        }
        ${
          this.#request.referrerPolicy()
            ? this.#renderGeneralRow(i18nString(UIStrings.referrerPolicy), String(this.#request.referrerPolicy()))
            : LitHtml.nothing
        }
      </${Category.litTagName}>
    `;
  ...
  #renderGeneralRow(name, value, classNames) {
    const isHighlighted =
      this.#toReveal?.section === NetworkForward.UIRequestLocation.UIHeaderSection.General &&
