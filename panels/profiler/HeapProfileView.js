...
export class HeapProfileView extends ProfileView {
  ...
  constructor(profileHeader) {
    ...
    if (Root.Runtime.experiments.isEnabled('samplingHeapProfilerTimeline')) {
      ...
      this.profileType.addEventListener(
        'StatsUpdate' /* SamplingHeapProfileType.Events.StatsUpdate */,
        this.onStatsUpdate,
        this,
      );
  ...
  onStatsUpdate(event) {
    ...
    for (const sample of profile?.samples ?? []) {
      ...
      const bucket =
        Platform.ArrayUtilities.upperBound(this.ordinals, sample.ordinal, Platform.ArrayUtilities.DEFAULT_COMPARATOR) -
        1;
...
export class SamplingHeapProfileHeader extends WritableProfileHeader {
  ...
  constructor(heapProfilerModel, type, title) {
    super(
      heapProfilerModel && heapProfilerModel.debuggerModel(),
      type,
