...
export class IsolateSelector extends UI.Widget.VBox {
  ...
  wasShown() {
    ...
    SDK.IsolateManager.IsolateManager.instance().addEventListener(
      SDK.IsolateManager.Events.MemoryChanged,
  ...
  isolateAdded(isolate) {
    ...
    const index =
      item.model().target() === SDK.TargetManager.TargetManager.instance().mainTarget() ? 0 : this.items.length;
