...
export class ProtocolMonitorImpl extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.dataGridIntegrator = new DataGrid.DataGridControllerIntegrator.DataGridControllerIntegrator(
      dataGridInitialData,
    );
  ...
  #createCommandInput() {
    ...
    const input = new UI.Toolbar.ToolbarInput(
      ...
      shrinkFactor,
      tooltip,
      this.#historyAutocompleteDataProvider.buildTextPromptCompletions,
      false,
    );
  ...
  render(data) {
    if (!data || !data.request || !data.response) {
      ...
      this.tabbedPane.changeTabView(
        'response',
        new UI.EmptyWidget.EmptyWidget(i18nString(UIStrings.noMessageSelected)),
      );
      return;
    }
