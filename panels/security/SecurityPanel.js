...
export class SecurityPanel extends UI.Panel.PanelWithSidebar {
  ...
  constructor() {
    ...
    this.sidebarMainViewElement = new SecurityPanelSidebarTreeElement(
      title,
      this.setVisibleView.bind(this, this.mainView),
...
export class SecurityPanelSidebarTree extends UI.TreeOutline.TreeOutlineInShadow {
  ...
  addOrigin(origin, securityState) {
    const originElement = new SecurityPanelSidebarTreeElement(
      ...
      this.showOriginInPanel.bind(this, origin),
...
export class SecurityMainView extends UI.Widget.VBox {
  ...
  constructor(panel) {
    ...
    this.lockSpectrum = new Map([
      [
        ...
        lockSpectrum.createChild('div', 'lock-icon lock-icon-secure'),
      ],
      [
        'neutral' /* Protocol.Security.SecurityState.Neutral */,
        lockSpectrum.createChild('div', 'lock-icon lock-icon-neutral'),
      ],
      [
        'insecure' /* Protocol.Security.SecurityState.Insecure */,
        lockSpectrum.createChild('div', 'lock-icon lock-icon-insecure'),
      ],
    ]);
  ...
  addExplanation(parent, explanation) {
    ...
    explanationSection
      .createChild('div', 'security-property')
  ...
  updateVisibleSecurityState(visibleSecurityState) {
    ...
    if (this.securityState === 'insecure' /* Protocol.Security.SecurityState.Insecure */) {
      this.getLockSpectrumDiv('insecure' /* Protocol.Security.SecurityState.Insecure */).classList.add(
        'lock-icon-insecure',
      );
      this.getLockSpectrumDiv('insecure' /* Protocol.Security.SecurityState.Insecure */).classList.remove(
  ...
  explainSafetyTipSecurity(visibleSecurityState, summary, explanations) {
    ...
    if (securityStateIssueIds.includes('bad_reputation')) {
      const formatedDescription = `${i18nString(UIStrings.chromeHasDeterminedThatThisSiteS)}\n\n${i18nString(
        UIStrings.ifYouBelieveThisIsShownIn,
      )}`;
      ...
      const formatedDescriptionSafety = `${i18nString(
        UIStrings.thisSitesHostnameLooksSimilarToP,
        hostnamePlaceholder,
  ...
  explainConnectionSecurity(visibleSecurityState, explanations) {
    ...
    if (certificateSecurityState.obsoleteSslProtocol) {
      recommendations.push(
        i18nString(UIStrings.sIsObsoleteEnableTlsOrLater, { PH1: certificateSecurityState.protocol }),
      );
    }
...
export class SecurityOriginView extends UI.Widget.VBox {
  ...
  constructor(panel, origin, originState) {
    ...
    if (originState.securityDetails) {
      ...
      table.addRow(
        '',
        SecurityPanel.createCertificateViewerButtonForOrigin(i18nString(UIStrings.openFullCertificateDetails), origin),
      );
      if (!sctSection) {
        return;
      }
