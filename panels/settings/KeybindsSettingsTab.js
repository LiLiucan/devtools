    ...
    for (const [originalShortcut, newDescriptors] of editedShortcuts) {
      ...
      if (newDescriptors) {
        UI.ShortcutRegistry.ShortcutRegistry.instance().registerUserShortcut(
          originalShortcut.changeKeys(newDescriptors).changeType(UI.KeyboardShortcut.Type.UserShortcut),
        );
        if (originalShortcut.type === UI.KeyboardShortcut.Type.UnsetShortcut) {
...
export class ShortcutListItem {
  ...
  setupEditor() {
    ...
    this.element.appendChild(
      this.createIconButton(i18nString(UIStrings.discardChanges), 'largeicon-delete', 'keybinds-cancel-button', () =>
  ...
  addShortcut() {
    const shortcut = new UI.KeyboardShortcut.KeyboardShortcut(
      [],
      this.item.id(),
      UI.KeyboardShortcut.Type.UnsetShortcut,
    );
  ...
  createEditButton() {
    return this.createIconButton(i18nString(UIStrings.editShortcut), 'largeicon-edit', 'keybinds-edit-button', () =>
