...
export class BreakpointsSidebarPane extends UI.ThrottledWidget.ThrottledWidget {
  ...
  constructor() {
    ...
    this.#breakpointsView.addEventListener(
      SourcesComponents.BreakpointsView.CheckboxToggledEvent.eventName,
      (event) => {
        const {
          data: { breakpointItem, checked },
        } = event;
    ...
    this.#breakpointsView.addEventListener(
      SourcesComponents.BreakpointsView.BreakpointSelectedEvent.eventName,
      (event) => {
        const {
          data: { breakpointItem },
        } = event;
    ...
    this.#breakpointsView.addEventListener(
      SourcesComponents.BreakpointsView.BreakpointEditedEvent.eventName,
      (event) => {
        const {
          data: { breakpointItem },
        } = event;
    ...
    this.#breakpointsView.addEventListener(
      SourcesComponents.BreakpointsView.BreakpointsRemovedEvent.eventName,
      (event) => {
        const {
          data: { breakpointItems },
        } = event;
    ...
    this.#breakpointsView.addEventListener(
      SourcesComponents.BreakpointsView.ExpandedStateChangedEvent.eventName,
      (event) => {
        const {
          data: { url, expanded },
        } = event;
...
export class BreakpointsSidebarController {
  ...
  #onBreakpointAdded(event) {
    ...
    if (
      breakpoint.origin === 'USER_ACTION' /* Bindings.BreakpointManager.BreakpointOrigin.USER_ACTION */ &&
  ...
  #getBreakpointState(locations) {
    ...
    if (hasEnabled) {
      status = hasDisabled
        ? 'INDETERMINATE' /* SourcesComponents.BreakpointsView.BreakpointStatus.INDETERMINATE */
