...
export class DebuggerPausedMessage {
  ...
  constructor() {
    ...
    const root = UI.Utils.createShadowRootWithCoreStyles(this.elementInternal, {
      cssFile: [debuggerPausedMessageStyles],
    ...
    if (data.targetNode) {
      ...
      if (data.insertion) {
        if (data.targetNode === data.node) {
          ...
          messageElement = i18n.i18n.getFormatLocalizedString(str_, UIStrings.descendantSAdded, {
            PH1: targetNodeLink,
          });
        }
      } else {
        messageElement = i18n.i18n.getFormatLocalizedString(str_, UIStrings.descendantSRemoved, {
          PH1: targetNodeLink,
        });
      }
  ...
  async render(details, debuggerWorkspaceBinding, breakpointManager) {
    ...
    if (details.reason === 'DOM' /* Protocol.Debugger.PausedEventReason.DOM */) {
      ...
      messageWrapper = buildWrapper(
        i18nString(UIStrings.pausedOnPromiseRejection),
      ...
      if (text === 'trustedtype-sink-violation' /* Protocol.DOMDebugger.CSPViolationType.TrustedtypeSinkViolation */) {
        messageWrapper = buildWrapper(
          i18nString(UIStrings.pausedOnCspViolation),
          i18nString(UIStrings.trustedTypeSinkViolation),
        );
      } else if (
        text === 'trustedtype-policy-violation' /* Protocol.DOMDebugger.CSPViolationType.TrustedtypePolicyViolation */
      ) {
        messageWrapper = buildWrapper(
          i18nString(UIStrings.pausedOnCspViolation),
          i18nString(UIStrings.trustedTypePolicyViolation),
        );
      }
