...
export class JavaScriptBreakpointsSidebarPane extends UI.ThrottledWidget.ThrottledWidget {
  ...
  async doUpdate() {
    ...
    for (let idx = 0; idx < locationsGroupedById.length; idx++) {
      ...
      const showColumn =
        uiLocation.uiSourceCode.mimeType() !== 'application/wasm' &&
  ...
  createElementForItem(item) {
    ...
    if (item.text && lineNumber < item.text.lineCount()) {
      ...
      snippetElement.textContent = Platform.StringUtilities.trimEndWithMaxLength(
        lineText.substring(item.showColumn ? uiLocation.columnNumber || 0 : 0),
        maxSnippetLength,
      );
    }
  ...
  breakpointCheckboxClicked(event) {
    ...
    for (const breakpoint of breakpoints) {
      ...
      const item = this.breakpoints.find((breakpointItem) =>
  ...
  revealLocation(element) {
    const uiLocations = this.breakpointLocationsForElement(element).map(
  ...
  breakpointContextMenu(event) {
    ...
    const removeEntryTitle =
      breakpoints.length > 1
        ? i18nString(UIStrings.removeAllBreakpointsInLine)
        : i18nString(UIStrings.removeBreakpoint);
    contextMenu
      .defaultSection()
      .appendItem(removeEntryTitle, () =>
        breakpoints.map((breakpoint) => void breakpoint.remove(false /* keepInStorage */)),
      );
    if (event.target instanceof Element) {
      contextMenu
        .defaultSection()
        .appendItem(i18nString(UIStrings.revealLocation), this.revealLocation.bind(this, event.target));
    }
    ...
    const breakpointActiveTitle = breakpointActive
      ? i18nString(UIStrings.deactivateBreakpoints)
      : i18nString(UIStrings.activateBreakpoints);
    contextMenu
      .defaultSection()
    ...
    if (breakpoints.some((breakpoint) => !breakpoint.enabled())) {
      ...
      if (event.target instanceof Element) {
        ...
        contextMenu
          .defaultSection()
          .appendItem(enableInFileTitle, this.toggleAllBreakpointsInFile.bind(this, event.target, true));
      }
    }
    if (breakpoints.some((breakpoint) => breakpoint.enabled())) {
      ...
      if (event.target instanceof Element) {
        ...
        contextMenu
          .defaultSection()
          .appendItem(disableInFileTitle, this.toggleAllBreakpointsInFile.bind(this, event.target, false));
      }
    }
    ...
    contextMenu
      .defaultSection()
      .appendItem(removeOtherTitle, this.removeOtherBreakpoints.bind(this, new Set(breakpoints)));
