...
export class ScriptFormatterEditorAction {
  ...
  editorSelected(event) {
    ...
    if (
      this.isFormattableScript(uiSourceCode) &&
      this.pathsToFormatOnLoad.has(uiSourceCode.url()) &&
  ...
  async editorClosed(event) {
    ...
    const original = await FormatterModule.SourceFormatter.SourceFormatter.instance().discardFormattedUISourceCode(
      uiSourceCode,
    );
    if (original) {
  ...
  async showFormatted(uiSourceCode) {
    ...
    this.sourcesView.showSourceLocation(formatData.formattedSourceCode, {
      lineNumber: start[0],
