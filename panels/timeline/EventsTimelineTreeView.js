...
export class EventsTimelineTreeView extends TimelineTreeView {
  ...
  showDetailsForNode(node) {
    ...
    void TimelineUIUtils.buildTraceEventDetails(traceEvent, model.timelineModel(), this.linkifier, false).then(
      (fragment) => this.detailsView.element.appendChild(fragment),
    );
    return true;
  }
  onHover(node) {
