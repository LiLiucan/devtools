...
export class PerformanceModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  async setTracingModel(model) {
    ...
    const mainTracks = this.timelineModelInternal
      ...
      .filter(
        (track) =>
          track.type === TimelineModel.TimelineModel.TrackType.MainThread && track.forMainFrame && track.events.length,
      );
    ...
    for (const entry of this.extensionTracingModels) {
      entry.model.adjustTime(
        this.tracingModelInternal.minimumRecordTime() + entry.timeOffset / 1000 - this.recordStartTimeInternal,
      );
    }
  ...
  async addSourceMapListeners() {
    ...
    for (const debuggerModel of debuggerModelsToListen) {
      debuggerModel
        .sourceMapManager()
        .addEventListener(SDK.SourceMapManager.Events.SourceMapAttached, this.#onAttachedSourceMap, this);
    }
