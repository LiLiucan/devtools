...
export class TimelineDetailsView extends UI.Widget.VBox {
  ...
  constructor(delegate) {
    ...
    this.defaultDetailsContentElement = this.defaultDetailsWidget.element.createChild(
      'div',
      'timeline-details-view-body vbox',
    );
  ...
  setSelection(selection) {
    ...
    switch (this.selection.type()) {
      ...
      case TimelineSelection.Type.NetworkRequest: {
        ...
        void TimelineUIUtils.buildNetworkRequestDetails(
          request,
          this.model.timelineModel(),
          this.detailsLinkifier,
  ...
  layersView() {
    ...
    this.lazyLayersView = new TimelineLayersView(
      this.model.timelineModel(),
      this.showSnapshotInPaintProfiler.bind(this),
    );
    return this.lazyLayersView;
  }
  paintProfilerView() {
  ...
  appendDetailsTabsForTraceEventAndShowDetails(event, content) {
    ...
    if (
      event.name === TimelineModel.TimelineModel.RecordType.Paint ||
  ...
  updateSelectedRangeStats(startTime, endTime) {
    ...
    contentHelper.addSection(
      i18nString(UIStrings.rangeSS, {
        PH1: i18n.TimeUtilities.millisToString(startOffset),
        PH2: i18n.TimeUtilities.millisToString(endOffset),
      }),
    );
