...
export class TimelineFlameChartNetworkDataProvider {
  ...
  prepareHighlightedEntryInfo(index) {
    ...
    if (startTime && isFinite(duration)) {
      contents.createChild('span', 'timeline-info-network-time').textContent = i18n.TimeUtilities.millisToString(
        duration,
        true,
      );
    }
  ...
  updateTimelineData() {
    ...
    this.timelineDataInternal = new PerfUI.FlameChart.TimelineData(
      this.timelineDataInternal.entryLevels,
      this.timelineDataInternal.entryTotalTimes,
      this.timelineDataInternal.entryStartTimes,
      [this.group],
    );
