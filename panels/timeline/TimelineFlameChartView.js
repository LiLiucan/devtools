...
class MainSplitWidget extends UI.SplitWidget.SplitWidget {
  ...
  setModelAndUpdateBoundaries(model) {
    ...
    const lcpEvents = events.filter(
      (e) => timelineModel.isLCPCandidateEvent(e) || timelineModel.isLCPInvalidateEvent(e),
    );
    ...
    const longTasks = events
      .filter((e) => SDK.TracingModel.TracingModel.isCompletePhase(e.phase) && timelineModel.isLongRunningTask(e))
    ...
    this.webVitals.webVitalsTimeline.data = {
      ...
      fcps: events
        .filter((e) => timelineModel.isFCPEvent(e))
        .map((e) => ({ timestamp: e.startTime - minimumBoundary, e })),
...
export class TimelineFlameChartView extends UI.Widget.VBox {
  ...
  constructor(delegate) {
    ...
    const mainViewGroupExpansionSetting = Common.Settings.Settings.instance().createSetting(
    ...
    this.networkFlameChartGroupExpansionSetting = Common.Settings.Settings.instance().createSetting(
      'timelineFlamechartNetworkViewGroupExpansion',
      {},
    );
  ...
  highlightEvent(event) {
    const entryIndex = event
      ? this.mainDataProvider.entryIndexForSelection(TimelineSelection.fromTraceEvent(event))
      : -1;
    if (entryIndex >= 0) {
  ...
  resizeToPreferredHeights() {
    ...
    this.networkSplitWidget.setSidebarSize(
      this.networkDataProvider.preferredHeight() + this.splitResizer.clientHeight + PerfUI.FlameChart.HeaderHeight + 2,
    );
  }
