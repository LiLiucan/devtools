...
export class TimelineHistoryManager {
  ...
  static previewElement(performanceModel) {
    ...
    data.time.textContent = startedAt
      ? i18nString(UIStrings.sAgo, { PH1: TimelineHistoryManager.coarseAge(startedAt) })
      : '';
    return data.preview;
  }
  ...
  buildTextDetails(performanceModel, title, timeElement) {
    ...
    const duration = i18n.TimeUtilities.millisToString(
...
export class DropDown {
  ...
  constructor(models) {
    ...
    this.glassPane.setPointerEventsBehavior(
      'BlockedByGlassPane' /* UI.GlassPane.PointerEventsBehavior.BlockedByGlassPane */,
    );
