...
export class WebVitalsEventLane extends WebVitalsLane {
  ...
  #renderLabel(position, label, textMetrics) {
    ...
    this.context.fillText(
      label,
      this.tX(position) + this.timeline.getLineHeight() * 0.5,
      0.5 * this.timeline.getLineHeight() + height * 0.5,
    );
    this.context.restore();
  }
  #renderTimestamp(position, textWidth, timestamp, textMetrics) {
    ...
    this.context.fillText(
      timestamp,
      this.tX(position) + this.timeline.getLineHeight() * 0.5 + textWidth + 5,
      0.5 * this.timeline.getLineHeight() + height * 0.5,
    );
    this.context.restore();
  }
    ...
    if (showLabel) {
      ...
      if (showDetails) {
        this.#renderTimestamp(
          marker.timestamp,
          labelMetrics ? labelMetrics.width : 0,
          timestampLabel,
          timestampMetrics,
        );
      }
