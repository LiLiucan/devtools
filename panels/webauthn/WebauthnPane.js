...
export class WebauthnPaneImpl extends UI.Widget.VBox {
  ...
  #createCredentialsDataGrid(authenticatorId) {
    ...
    dataGrid.addEventListener(
      'RemoveCredential' /* Events.RemoveCredential */,
      this.#handleRemoveCredential.bind(this, authenticatorId),
    );
  ...
  `);
    ...
    const newAuthenticatorTitle = UI.UIUtils.createLabel(
    ...
    this.addAuthenticatorButton = UI.UIUtils.createTextButton(
      ...
      this.#handleAddAuthenticatorButton.bind(this),
      '',
    );
  ...
  async #addAuthenticatorSection(authenticatorId, options) {
    ...
    const activeLabel = UI.UIUtils.createRadioLabel(
      `active-authenticator-${authenticatorId}`,
    ...
    editName.addEventListener(UI.Toolbar.ToolbarButton.Events.Click, () =>
      this.#handleEditNameButton(titleElement, nameField, editName, saveName),
    );
    saveName.addEventListener(UI.Toolbar.ToolbarButton.Events.Click, () =>
  ...
  #createAuthenticatorFields(section, authenticatorId, options) {
    ...
    srkField.createChild('div', 'authenticator-field-value').textContent = options.hasResidentKey
      ? i18nString(UIStrings.yes)
      : i18nString(UIStrings.no);
    slbField.createChild('div', 'authenticator-field-value').textContent = options.hasLargeBlob
      ? i18nString(UIStrings.yes)
      : i18nString(UIStrings.no);
    suvField.createChild('div', 'authenticator-field-value').textContent = options.hasUserVerification
      ? i18nString(UIStrings.yes)
      : i18nString(UIStrings.no);
  }
  ...
  #updateActiveLabelTitle(activeLabel, authenticatorName) {
    UI.Tooltip.Tooltip.install(
      activeLabel.radioElement,
      i18nString(UIStrings.setSAsTheActiveAuthenticator, { PH1: authenticatorName }),
    );
  }
  /**
