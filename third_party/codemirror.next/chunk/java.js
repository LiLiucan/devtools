...
const Z = O({
    ...
    'class interface extends implements enum': $.definitionKeyword,
      ...
      props: [
        a.add({
          ...
          SwitchBlock: (O) => {
            let $ = O.textAfter,
              Q = /^\s*\}/.test($),
              P = /^\s*(case|default)\b/.test($);
            return O.baseIndent + (Q ? 0 : P ? 1 : 2) * O.unit;
          },
          Block: r({ closing: '}' }),
          BlockComment: () => -1,
          Statement: e({ except: /^{/ }),
...
export { n as java, Y as javaLanguage };
