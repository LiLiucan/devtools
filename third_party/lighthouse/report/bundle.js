...
const UIStrings = {
  ...
  runtimeSingleLoadTooltip:
    'This data is taken from a single page load, as opposed to field data summarizing many sessions.', // eslint-disable-line max-len
...
function createTopbarComponent(dom) {
  ...
  el18.setAttribute(
    'd',
    'M12 3l4.125 2.625v3.75H18v2.25h-1.688l1.5 9.375H6.188l1.5-9.375H6v-2.25h1.875V5.648L12 3zm2.201 9.938L9.54 14.633 9 18.028l5.625-2.062-.424-3.028zM12.005 5.67l-1.88 1.207v2.498h3.75V6.86l-1.87-1.19z',
  );
...
class CategoryRenderer {
  ...
  injectFinalScreenshot(categoriesEl, audits, scoreScaleEl) {
    ...
    const separatorEl = this.dom.createElement('div', 'lh-category-headercol lh-category-headercol--separator');
...
class CriticalRequestChainRenderer {
  ...
  static render(dom, details, detailsRenderer) {
    ...
    dom.find('.lh-crc__longest_duration_label', tmpl).textContent = Util.i18n.strings.crcLongestDurationLabel;
    dom.find('.lh-crc__longest_duration', tmpl).textContent = Util.i18n.formatMilliseconds(
...
class PerformanceCategoryRenderer extends CategoryRenderer {
  ...
  render(category, groups, options) {
    ...
    if (opportunityAudits.length) {
      ...
      opportunityAudits.forEach((item) => groupEl.insertBefore(this._renderOpportunity(item, scale), footerEl));
    ...
    const passedAudits = category.auditRefs.filter(
      (audit) => this._classifyPerformanceAudit(audit) && Util.showAsPassed(audit.result),
    );
  ...
  renderMetricAuditFilter(filterableMetrics, categoryEl) {
    ...
    const filterChoices = /** @type {LH.ReportResult.AuditRef[]} */ ([{ acronym: 'All' }, ...filterableMetrics]);
...
class ReportRenderer {
  ...
  _renderMetaBlock(report, footer) {
    ...
    const chromeVer = Array.isArray(match) ? match[1].replace('/', ' ').replace('Chrome', 'Chromium') : 'Chromium';
    ...
    const metaItems = [
      ...
      ['samples-one', Util.i18n.strings.runtimeSingleLoad, Util.i18n.strings.runtimeSingleLoadTooltip],
      ['stopwatch', Util.i18n.strings.runtimeAnalysisWindow],
      [
        'networkspeed',
        `${envValues.summary}`,
  ...
  _renderScoreGauges(report, categoryRenderer, specificCategoryRenderers) {
    ...
    for (const category of Object.values(report.categories)) {
      ...
      const gaugeWrapperEl = this._dom.find('a.lh-gauge__wrapper, a.lh-fraction__wrapper', categoryGauge);
      if (gaugeWrapperEl) {
  ...
  _renderReport(report) {
    ...
    if (scoreHeader) {
      ...
      scoreHeader.append(...this._renderScoreGauges(report, categoryRenderer, specificCategoryRenderers));
      scoresContainer.appendChild(scoreHeader);
      scoresContainer.appendChild(scoreScale);

      const stickyHeader = this._dom.createElement('div', 'lh-sticky-header');
      stickyHeader.append(...this._renderScoreGauges(report, categoryRenderer, specificCategoryRenderers));
    ...
    if (fullPageScreenshot) {
      ElementScreenshotRenderer.installFullPageScreenshot(this._dom.rootEl, fullPageScreenshot.screenshot);
    }
...
class DropDownMenu {
  ...
  _getNextSelectableNode(allNodes, startNode) {
    const nodes = allNodes.filter(
      /** @return {node is HTMLElement} */ (node) => {
...
class TopbarFeatures {
  ...
  _setupStickyHeader() {
    ...
    window.requestAnimationFrame(() =>
      window.requestAnimationFrame(() => {
        ...
        const scrollParent = this._getScrollParent(this._dom.find('.lh-container', this._dom.rootEl));
        ...
        const resizeTarget = scrollParent instanceof window.Document ? document.documentElement : scrollParent;
  ...
  _updateStickyHeader() {
    ...
    const highlightIndex = categoriesAboveTheMiddle.length > 0 ? categoriesAboveTheMiddle.length - 1 : 0;

    // Category order matches gauge order in sticky header.
    const gaugeWrapperEls = this.stickyHeaderEl.querySelectorAll('.lh-gauge__wrapper, .lh-fraction__wrapper');
...
class ReportUIFeatures {
  ...
  initFeatures(lhr) {
    ...
    const disableDarkMode =
      this._dom.isDevTools() || this._opts.disableDarkMode || this._opts.disableAutoDarkModeAndFireworks;
    ...
    const disableFireworks = this._opts.disableFireworks || this._opts.disableAutoDarkModeAndFireworks;
    ...
    const showTreemapApp = this.json.audits['script-treemap-data'] && this.json.audits['script-treemap-data'].details;
    if (showTreemapApp) {
      this.addButton({
  ...
  _setupThirdPartyFilter() {
    ...
    tablesWithUrls.forEach((tableEl) => {
      ...
      this._dom.find('.lh-3p-filter-count', filterTemplate).textContent = `${thirdPartyRows.length}`;
      this._dom.find('.lh-3p-ui-string', filterTemplate).textContent = Util.i18n.strings.thirdPartyResourcesLabel;
