...
export class ChromeTargetManager extends EventEmitter {
  constructor(connection, targetFactory, targetFilterCallback) {
    ...
    _ChromeTargetManager_storeExistingTargetsForInit.set(this, () => {
      for (const [targetId, targetInfo] of __classPrivateFieldGet(
        this,
        _ChromeTargetManager_discoveredTargetsByTargetId,
        'f',
      ).entries()) {
        if (
          (!__classPrivateFieldGet(this, _ChromeTargetManager_targetFilterCallback, 'f') ||
    ...
    _ChromeTargetManager_onTargetCreated.set(this, async (event) => {
      __classPrivateFieldGet(this, _ChromeTargetManager_discoveredTargetsByTargetId, 'f').set(
      ...
      if (event.targetInfo.type === 'browser' && event.targetInfo.attached) {
        if (
          __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsByTargetId, 'f').has(
            event.targetInfo.targetId,
          )
        ) {
          return;
        }
        const target = __classPrivateFieldGet(this, _ChromeTargetManager_targetFactory, 'f').call(
          this,
          event.targetInfo,
          undefined,
        );
        __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsByTargetId, 'f').set(
          event.targetInfo.targetId,
          target,
        );
      }
    ...
    _ChromeTargetManager_onTargetDestroyed.set(this, (event) => {
      const targetInfo = __classPrivateFieldGet(this, _ChromeTargetManager_discoveredTargetsByTargetId, 'f').get(
        event.targetId,
      );
      ...
      if (
        (targetInfo === null || targetInfo === void 0 ? void 0 : targetInfo.type) === 'service_worker' &&
        __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsByTargetId, 'f').has(event.targetId)
      ) {
        // Special case for service workers: report TargetGone event when
        // the worker is destroyed.
        const target = __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsByTargetId, 'f').get(
          event.targetId,
        );
    ...
    _ChromeTargetManager_onTargetInfoChanged.set(this, (event) => {
      ...
      if (
        __classPrivateFieldGet(this, _ChromeTargetManager_ignoredTargets, 'f').has(event.targetInfo.targetId) ||
        !__classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsByTargetId, 'f').has(
          event.targetInfo.targetId,
        ) ||
    ...
    _ChromeTargetManager_onAttachedToTarget.set(this, async (parentSession, event) => {
      ...
      if (
        ...
        await silentDetach();
        if (
          __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsByTargetId, 'f').has(targetInfo.targetId)
        ) {
          return;
        }
      ...
      const existingTarget = __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsByTargetId, 'f').has(
        targetInfo.targetId,
      );
      ...
      if (existingTarget) {
        __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsBySessionId, 'f').set(
          session.id(),
          __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsByTargetId, 'f').get(targetInfo.targetId),
        );
      } else {
      ...
      for (const interceptor of __classPrivateFieldGet(this, _ChromeTargetManager_targetInterceptors, 'f').get(
        parentSession,
      ) || []) {
        if (!(parentSession instanceof Connection)) {
          ...
          assert(
            __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsBySessionId, 'f').has(parentSession.id()),
          );
        }
        await interceptor(
          target,
          parentSession instanceof Connection
            ? null
            : __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsBySessionId, 'f').get(
                parentSession.id(),
              ),
        );
      }
    ...
    _ChromeTargetManager_onDetachedFromTarget.set(this, (_parentSession, event) => {
      const target = __classPrivateFieldGet(this, _ChromeTargetManager_attachedTargetsBySessionId, 'f').get(
        event.sessionId,
      );
    ...
    __classPrivateFieldGet(this, _ChromeTargetManager_connection, 'f').on(
      ...
      __classPrivateFieldGet(this, _ChromeTargetManager_onTargetInfoChanged, 'f'),
    );
    __classPrivateFieldGet(this, _ChromeTargetManager_connection, 'f').on(
      'sessiondetached',
    ...
    __classPrivateFieldGet(
      ...
      'm',
      _ChromeTargetManager_setupAttachmentListeners,
    ).call(this, __classPrivateFieldGet(this, _ChromeTargetManager_connection, 'f'));
  ...
  removeTargetInterceptor(client, interceptor) {
    ...
    __classPrivateFieldGet(this, _ChromeTargetManager_targetInterceptors, 'f').set(
      client,
...
(_ChromeTargetManager_connection = new WeakMap()),
  ...
  (_ChromeTargetManager_setupAttachmentListeners = function _ChromeTargetManager_setupAttachmentListeners(session) {
    ...
    __classPrivateFieldGet(this, _ChromeTargetManager_detachedFromTargetListenersBySession, 'f').set(
      session,
      detachedListener,
    );
  ...
  (_ChromeTargetManager_removeAttachmentListeners = function _ChromeTargetManager_removeAttachmentListeners(session) {
    if (__classPrivateFieldGet(this, _ChromeTargetManager_attachedToTargetListenersBySession, 'f').has(session)) {
      session.off(
        ...
        __classPrivateFieldGet(this, _ChromeTargetManager_attachedToTargetListenersBySession, 'f').get(session),
      );
    ...
    if (__classPrivateFieldGet(this, _ChromeTargetManager_detachedFromTargetListenersBySession, 'f').has(session)) {
      session.off(
        ...
        __classPrivateFieldGet(this, _ChromeTargetManager_detachedFromTargetListenersBySession, 'f').get(session),
      );
      __classPrivateFieldGet(this, _ChromeTargetManager_detachedFromTargetListenersBySession, 'f').delete(session);
    }
  }),
  (_ChromeTargetManager_finishInitializationIfReady = function _ChromeTargetManager_finishInitializationIfReady(
    targetId,
  ) {
    targetId !== undefined &&
