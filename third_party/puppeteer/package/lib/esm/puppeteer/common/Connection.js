...
export class Connection extends EventEmitter {
  constructor(url, transport, delay = 0) {
    ...
    __classPrivateFieldGet(this, _Connection_transport, 'f').onclose = __classPrivateFieldGet(
      this,
...
export class CDPSessionImpl extends CDPSession {
  ...
  send(method, ...paramArgs) {
    if (!__classPrivateFieldGet(this, _CDPSessionImpl_connection, 'f')) {
      return Promise.reject(
        new Error(
          `Protocol error (${method}): Session closed. Most likely the ${__classPrivateFieldGet(
            this,
            _CDPSessionImpl_targetType,
            'f',
          )} has been closed.`,
  ...
  _onMessage(object) {
    const callback = object.id
      ? __classPrivateFieldGet(this, _CDPSessionImpl_callbacks, 'f').get(object.id)
      : undefined;
  ...
  async detach() {
    if (!__classPrivateFieldGet(this, _CDPSessionImpl_connection, 'f')) {
      throw new Error(
        `Session already detached. Most likely the ${__classPrivateFieldGet(
          this,
          _CDPSessionImpl_targetType,
          'f',
        )} has been closed.`,
...
(_CDPSessionImpl_sessionId = new WeakMap()),
  (_CDPSessionImpl_targetType = new WeakMap()),
  (_CDPSessionImpl_callbacks = new WeakMap()),
  (_CDPSessionImpl_connection = new WeakMap());
...
function rewriteError(error, message, originalMessage) {
  ...
  error.originalMessage =
    originalMessage !== null && originalMessage !== void 0 ? originalMessage : error.originalMessage;
  return error;
}
/**
 * @internal
