    ...
    __classPrivateFieldSet(
      ...
      [
        addEventListener(pipeRead, 'data', (buffer) => {
          return __classPrivateFieldGet(this, _PipeTransport_instances, 'm', _PipeTransport_dispatch).call(
            this,
            buffer,
          );
        }),
...
(_PipeTransport_pipeWrite = new WeakMap()),
  (_PipeTransport_eventListeners = new WeakMap()),
  (_PipeTransport_isClosed = new WeakMap()),
  (_PipeTransport_pendingMessage = new WeakMap()),
  (_PipeTransport_instances = new WeakSet()),
  (_PipeTransport_dispatch = function _PipeTransport_dispatch(buffer) {
    assert(!__classPrivateFieldGet(this, _PipeTransport_isClosed, 'f'), '`PipeTransport` is closed.');
    ...
    if (end === -1) {
      __classPrivateFieldSet(
        this,
        _PipeTransport_pendingMessage,
        __classPrivateFieldGet(this, _PipeTransport_pendingMessage, 'f') + buffer.toString(),
        'f',
      );
      return;
    }
    const message =
      __classPrivateFieldGet(this, _PipeTransport_pendingMessage, 'f') + buffer.toString(undefined, 0, end);
    if (this.onmessage) {
