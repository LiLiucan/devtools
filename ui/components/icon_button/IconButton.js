...
export class IconButton extends HTMLElement {
  ...
  #render() {
    ...
    LitHtml.render(
      LitHtml.html`
      <button class=${buttonClasses} @click=${this.#onClickHandler} aria-label=${LitHtml.Directives.ifDefined(
        this.#accessibleName,
      )}>
      ${
        !this.#compact && this.#leadingText
          ? LitHtml.html`<span class="icon-button-title">${this.#leadingText}</span>`
          : LitHtml.nothing
      }
      ${filteredGroups.map(
        (counter) => LitHtml.html`
      <${Icon.litTagName} class="status-icon"
      .data=${{
        iconName: counter.iconName,
        color: counter.iconColor,
        width: counter.iconWidth || '1.5ex',
        height: counter.iconHeight || '1.5ex',
      }}>
      </${Icon.litTagName}>
      ...
      ${
        !this.#compact && this.#trailingText
          ? LitHtml.html`<span class="icon-button-title">${this.#trailingText}</span>`
          : LitHtml.nothing
      }
    `,
      this.#shadow,
