...
const str_ = i18n.i18n.registerUIStrings(
  'ui/components/linear_memory_inspector/LinearMemoryInspectorController.ts',
  UIStrings,
);
...
export class LinearMemoryInspectorController extends SDK.TargetManager.SDKModelObserver {
  ...
  async openInspectorView(obj, address, expression) {
    ...
    if (memoryAddress !== undefined) {
      ...
      console.assert(
        memoryObj.subtype === 'webassemblymemory' /* Protocol.Runtime.RemoteObjectSubtype.Webassemblymemory */,
      );
  ...
  static extractHighlightInfo(obj, expression) {
    ...
    const startAddress =
      (obj instanceof Bindings.DebuggerLanguagePlugins.ExtensionRemoteObject
        ? obj.linearMemoryAddress
        : obj.inspectableAddress) ?? 0;
    let highlightInfo;
    try {
      highlightInfo = {
        startAddress,
