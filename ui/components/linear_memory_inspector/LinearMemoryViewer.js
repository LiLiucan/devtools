...
export class LinearMemoryViewer extends HTMLElement {
  ...
  #render() {
    ...
    // clang-format off
    render(
      html` <div class="view" tabindex="0" @keydown=${this.#onKeyDown}>${this.#renderView()}</div> `,
      this.#shadow,
      { host: this },
    );
  }
  ...
  #renderRow(row) {
    ...
    return html`
      <div class="row">
        <span class=${LitHtml.Directives.classMap(classMap)}
  ...
  #renderByteValues(startIndex, endIndex) {
    ...
    for (let i = startIndex; i < endIndex; ++i) {
      ...
      cells.push(
        html`<span class=${LitHtml.Directives.classMap(classMap)} @click=${onSelectedByte}>${byteValue}</span>`,
      );
    }
    return html`${cells}`;
  }
  ...
  #shouldBeHighlighted(index) {
    ...
    return (
      this.#highlightInfo.startAddress <= index && index < this.#highlightInfo.startAddress + this.#highlightInfo.size
    );
  }
  #isFocusedArea(index) {
