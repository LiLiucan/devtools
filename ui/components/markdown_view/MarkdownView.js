...
export class MarkdownView extends HTMLElement {
  ...
  #render() {
    ...
    // clang-format off
    render(html` <div class="message">${this.#tokenData.map(renderToken)}</div> `, this.#shadow, { host: this });
    // clang-format on
  }
}
...
function templateForToken(token) {
  switch (token.type) {
    case 'paragraph':
      return html`<p>${renderChildTokens(token)}</p>`;
    case 'list':
      return html`<ul>
        ${token.items.map(renderToken)}
      </ul>`;
    case 'list_item':
      return html`<li>${renderChildTokens(token)}</li>`;
    case 'text':
    ...
    case 'space':
      return html``;
    case 'link':
      return html`<${MarkdownLink.litTagName} .data=${{ key: token.href, title: token.text }}></${
        MarkdownLink.litTagName
      }>`;
    case 'image':
      return html`<${MarkdownImage.litTagName} .data=${{ key: token.href, title: token.text }}></${
