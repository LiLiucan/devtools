...
export function guessIndent(doc) {
  ...
  let scanned = 0;
  for (
    let cur = doc.iterLines(1, Math.min(doc.lines + 1, LINES_TO_SCAN_FOR_INDENTATION_GUESSING));
    !cur.next().done;
...
export const closeBrackets = [CM.closeBrackets(), CM.keymap.of(CM.closeBracketsKeymap)];
