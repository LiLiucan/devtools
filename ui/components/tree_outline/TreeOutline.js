...
export class TreeOutline extends HTMLElement {
  ...
  #defaultRenderer = (node, _state) => {
    if (typeof node.treeNodeData !== 'string') {
      console.warn(
        `The default TreeOutline renderer simply stringifies its given value. You passed in ${JSON.stringify(
          node.treeNodeData,
          null,
          2,
  ...
  #renderNode(node, { depth, setSize, positionInSet }) {
    ...
    const ariaExpandedAttribute = LitHtml.Directives.ifDefined(
      isExpandableNode(node) ? String(nodeIsExpanded) : undefined,
    );
    let renderedNodeKey;
    if (node.renderer) {
