...
export class BezierEditor extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  dragStart(event) {
    ...
    const firstControlPointIsCloser =
      this.controlPosition.distanceTo(this.bezierInternal.controlPoints[0]) <
  ...
  presetModifyClicked(intensify, _event) {
    ...
    const newBezier = UI.Geometry.CubicBezier.parse(
      this.selectedCategory.presets[this.selectedCategory.presetIndex].value,
    );
    if (newBezier) {
      this.setBezier(newBezier);
      this.onchange();
