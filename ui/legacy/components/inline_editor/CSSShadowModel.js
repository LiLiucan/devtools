...
export class CSSShadowModel {
  ...
  static parseShadow(text, isBoxShadow) {
    ...
    for (let i = 0; i < shadowTexts.length; i++) {
      ...
      if (
        invalidCount(shadow, 'X' /* Part.OffsetX */, 1, 1) ||
        invalidCount(shadow, 'Y' /* Part.OffsetY */, 1, 1) ||
        invalidCount(shadow, 'C' /* Part.Color */, 0, 1) ||
        invalidCount(shadow, 'B' /* Part.BlurRadius */, 0, 1) ||
